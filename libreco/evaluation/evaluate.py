    ...
    if model.task == "rating":
        ...
        if train_data:
            ...
            if POINTWISE_METRICS.intersection(metrics):
                train_params["y_prob"], train_params["y_true"] = compute_probs(
                    model, train_data, eval_batch_size
                )
        ...
        if eval_data:
            test_params = dict()
            if POINTWISE_METRICS.intersection(metrics):
                test_params["y_prob"], test_params["y_true"] = compute_probs(
                    model, eval_data, eval_batch_size
                )
