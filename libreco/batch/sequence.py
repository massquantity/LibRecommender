...
def get_interacted_seq(
    ...
    batch_interacted = np.full((batch_size, max_seq_len), pad_index, dtype=np.int32)
    ...
    for j, (u, i) in enumerate(zip(user_indices, item_indices)):
        ...
        if position == 0:
            ...
            batch_interacted_len.append(max_seq_len)

    return batch_interacted, np.array(batch_interacted_len, dtype=np.int32)
