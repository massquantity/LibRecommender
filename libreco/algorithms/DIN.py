...
class Din(BaseFeat):
    ...
    def build_model(self, dataset):
        ...
        self.seq_matrix = tf.placeholder(tf.int32, shape=[None, None])
                ...
                #    t4 = time.time()
                    for n in range(n_batches):
                        ...
                        self.sess.run(self.training_op, feed_dict={self.feature_indices: indices_batch,
                                                                   ...
                                                                   self.is_training: True})
                    #    if n % 10 == 0:
                    #        print("batch %d: %.4f" % (n, time.time() - t4))
    ...
    #    max_seq_len = keys.get_shape().as_list()[1]
        ...
        layer_one = tf.layers.dense(din_all, 10, activation=tf.nn.sigmoid)
        ...
        outputs = outputs / (self.embed_size ** 0.5)
