...
class userKNN_909:
    ...
    def evaluate(self, dataset, num):
        ...
        for user, item in zip(user_indices, item_indices):
            ...
            if len(sim_indices) == 0 or sum(sims[sim_indices]) == 0:
...
class userKNN:
    ...
    def fit(self, dataset):
        ...
        faiss.normalize_L2(self.m)
        self.sim_model = faiss.IndexFlatIP(dataset.n_items)
        self.sim_model.train(self.m)
        self.sim_model.add(self.m)
        print(self.sim_model.is_trained)
        return self
    ...
    def evaluate(self, dataset, num):
        ...
        t2 = time.time()
        sims_all, neighbors_indices_all = self.sim_model.search(self.m, self.n_users)  # add min_support
    #    print(sims_all[:5, :10], neighbors_indices_all[:5, :10])
        print("faiss time: ", time.time() - t2, "\tsim shape: ", sims_all.shape)
        pred = []
        for user, item in zip(user_indices, item_indices):
            sims = sims_all[user]  # [1:]
