...
class ALS_ranking_67876:
    ...
    def fit(self, dataset, verbose=1):
        ...
        for epoch in range(1, self.n_epochs + 1):
            t0 = time.time()
            YtY = self.Y.T.dot(self.Y) + self.reg * np.eye(self.n_factors)
            for u in dataset.train_user:
                ...
                ycy_reg = YtY + np.dot(self.Y.T, Cui.dot(self.Y))
            ...
            XtX = self.X.T.dot(self.X) + self.reg * np.eye(self.n_factors)
            for i in dataset.train_item:
                ...
                xcx_reg = XtX + np.dot(self.X.T, Cii.dot(self.X))
