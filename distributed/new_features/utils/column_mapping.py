...
def col_name2index(sparse_col, dense_col=None, user_col=None, item_col=None):
    name_mapping = defaultdict(OrderedDict)   # {column_family_name: {column_name: index}}
    ...
    if user_col:
        ...
        for col in user_sparse_col:
            name_mapping["user_sparse_col"].update({col: name_mapping["sparse_col"][col]})
    ...
    if item_col:
        ...
        for col in item_sparse_col:
            name_mapping["item_sparse_col"].update({col: name_mapping["sparse_col"][col]})
...
def _extract_common_col(col1, col2):
    ...
    # but we want to preserve the original order of common column in col1 and col2
    common_col, indices_in_col1, _ = np.intersect1d(col1, col2, assume_unique=True, return_indices=True)
