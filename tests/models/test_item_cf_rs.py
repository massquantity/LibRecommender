...
def test_retrain():
    ...
    data_info.save(path=SAVE_PATH, model_name="itemcf_model")
    model.save(
        path=SAVE_PATH, model_name="itemcf_model", manual=True, inference_only=False
    )

    # ========================== load and retrain =============================
    new_data_info = DataInfo.load(SAVE_PATH, model_name="itemcf_model")
    ...
    new_model.rebuild_model(path=SAVE_PATH, model_name="itemcf_model")
    new_model.fit(
        train_data,
        neg_sampling=True,
        verbose=2,
    ...
    new_data_info.save(path=SAVE_PATH, model_name="itemcf_model")
    new_model.save(
        path=SAVE_PATH, model_name="itemcf_model", manual=True, inference_only=False
    )
