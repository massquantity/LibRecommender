...
def test_fm(
    ...
    if neg_sampling is None:
        with pytest.raises(AssertionError):
            FM(task, data_info).fit(train_data, neg_sampling)
    elif task == "rating" and neg_sampling:
        with pytest.raises(ValueError):
            FM(task, data_info).fit(train_data, neg_sampling)
    elif loss_type == "focal" and (neg_sampling is False or sampler is None):
        with pytest.raises(ValueError):
            FM(task, data_info, sampler=sampler).fit(train_data, neg_sampling)
    elif task == "ranking" and loss_type not in ("cross_entropy", "focal"):
        with pytest.raises(ValueError):
            FM(task, data_info, loss_type).fit(train_data, neg_sampling)
    elif num_workers < 0:
        with pytest.raises(ValueError):
            FM(task, data_info, loss_type).fit(
