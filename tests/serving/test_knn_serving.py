...
def test_knn_serving(knn_model, redis_client):
    ...
    response = requests.post(
        "http://localhost:8000/knn/recommend", json={"user": 1, "n_rec": 1}
    )
    assert len(list(response.json().values())[0]) == 1
    response = requests.post(
        "http://localhost:8000/knn/recommend", json={"user": 33, "n_rec": 3}
    )
